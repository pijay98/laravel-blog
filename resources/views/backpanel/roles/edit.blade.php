...
<div style = "position:relative; left:40px; top:80px;">
    <a href="{{route('role.index')}}" class="btn btn-primary rounded">Update</a>
</div>
<h3 class="text-center" style="padding-top:60px;">Update {{$roles->name}}</h3>
@if (count($errors) > 0)
...
<form action="{{route('role.update',$roles->id)}}" method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" id="name" class="form-control" name="name" value="{{$roles->name}}">
</div>
